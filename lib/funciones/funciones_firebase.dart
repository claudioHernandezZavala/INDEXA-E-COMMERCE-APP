...
Future<justGeneralInfo> getInfoForCotizacion() async {
  ...
  await reference.get().then((DocumentSnapshot snapshot) {
    if (snapshot.exists) {
      ...
      info = justGeneralInfo(
          data['Nombre'],
          snapshot.get('Direccion').cast<String>(),
          snapshot.get('Numero'),
          snapshot.get('token'));
    }
  });
...
void subirCarrito(Producto producto, int cantidad, BuildContext context) {
      ...
      if (querySnapshot.size == 0) {
        ...
        await referencia1.add(dataAgregar).whenComplete(() {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("Produto subido a carrito"),
              backgroundColor: Colors.green,
            ),
          );
        });
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Este producto ya esta en tu carrito"),
...
Future<infoUser> getInfo() async {
  ...
  DocumentReference reference =
      FirebaseFirestore.instance.collection("usuarios/").doc(user!.uid);
  infoUser info = infoUser('aaaaaa', [], 0, '', reference, [reference], false);
  await reference.get().then((DocumentSnapshot snapshot) {
    if (snapshot.exists) {
      final data = snapshot.data() as Map<String, dynamic>;
...
void sendNotification(String texto, titulo) async {
  try {
    await http.post(
      ...
      body: jsonEncode(
        <String, dynamic>{
          'notification': <String, dynamic>{'body': texto, 'title': '$titulo'},
          'priority': 'normal',
          'data': <String, dynamic>{
            'click_action': 'FLUTTER_NOTIFICATION_CLICK',
            'id': '1',
